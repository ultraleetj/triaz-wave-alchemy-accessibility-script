...
4. **Load kit** — genre → variant submenus; loaded variant marked with `!` checkmark (`_loaded_kit` module var). Always overwrites existing instances (no dialog). 16 genres / 39 variants.
...
Two functions:
- `_do_load_kit(track, genre_n, variant_n, used_wavs)` — inner loader; `used_wavs` table (path→true) prevents duplicate WAV across voices within one kit load; falls back to best-any only when type fully exhausted.
- `load_vibe_kit_flow(track, genre_n, variant_n)` — always clears existing instances first (no dialog; REAPER undo handles recovery), then calls `_do_load_kit` with fresh `{}`.

`_loaded_kit = {genre_n, variant_n}` — module-level var; set after each load; used by `show_main_menu` to prefix loaded variant with `!` checkmark in submenu.
...
`load_voice(track, drum_type, tag, notes, pitch_center, stats, opts)`:
- `opts.kw` — keyword list → `pick_by_keywords(drum_type, kw, used_wavs)`
- `opts.used_wavs` — shared dedup table; chosen path added after selection
- `opts.pitch_scale`, `opts.pan_list` — per-note pitch/pan overrides
...
### Vibe kits (VIBE_GENRES)

16 genres / 39 variants. Each variant: `{name, default_kw, voices={key={type,kw},...}}`.
Voice keys: `kick kick_alt snare snare_alt clap hh_c hh_pedal hh_o crash ride perc tom`.
`default_kw` used for rimshot, fixed GM voices, lower extras, upper zones.
...
### WAV keyword vocabulary (confirmed in library filenames)

Scoring (`pick_by_keywords`) matches against tag folder names AND WAV filename words.
Tag names: deep, sub, organic, room, punchy, tight, snap, heavy, lofi, tape, vinyl, acoustic, 808, machine, synthetic, layered, creative, noise, metallic, bright, mallet, etc.

Additional words confirmed in WAV filenames (useful as genre-specific keywords):
`funk` (88 hits), `tube` (61), `linn` (56), `707` (50), `606` (37), `disco` (47),
`djembe` (45), `balafon` (37), `miami` (35), `gold` (80), `dirty` (49), `vintage` (39), `lush` (39).

Genre names NOT in filenames (use character descriptors instead):
afro, jazz, latin, jungle, drill, amen, reggae, garage.
...
5. **Similar** — scores all WAVs in same drum type by keywords extracted from current filename; picks highest-scoring different WAV per voice
